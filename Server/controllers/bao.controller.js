...
exports.getComponent = async (req, res) => {
    try {
        const queryText = `SELECT "SubschemeId", "CompId","CompName","CompTypeId" 
                           FROM "ComponentMaster" 
                           WHERE "SubschemeId" = '${req.query.SubschemeId}' AND "Fin_Year" = '${req.query.Fin_Year}' AND "Active" = 1;`
        const result = await db.sequelize.query(queryText);
        res.send(result[0]);
    } catch (e) {
        res.status(500).send('Unexpected error');
...
exports.getDemonstrationData = async (req, res) => {
    try {
        ...
        WHERE a."Block_Code"='${req.payload.Block_Code}' AND a."CompId" = '${req.query.CompId}' AND a."Fin_Year" = '${req.query.Fin_Year}'`
        const result = await db.sequelize.query(queryText);
...
exports.getVerifiedDemonstrationData = async (req, res) => {
    try {
        ...
        LEFT JOIN "SubCropMaster" bb ON a."BP_SubCropId" = bb."SubCropId"
        LEFT JOIN "CropVarietyMaster" cc ON a."BP_Variety_Code" = cc."Variety_Code"
        WHERE a."Block_Code"='${req.payload.Block_Code}' AND a."SubschemeId" = '${req.query.SubschemeId}' 
        AND a."CompId" = '${req.query.CompId}' AND a."Fin_Year" = '${req.query.Fin_Year}'
        AND a."ConfirmBy_vaw" = '1' AND a."ConfirmBy_BAO" = '1'`
...
exports.getAllDealerSale = async (req, res, next) => {
    try {
        const queryText = `SELECT a."InvoiceNo", a."DemonstrationId" , a."Permit_NO" , a."FarmerId" ,a."FarmerName", a."LandArea", a."Farmer_Category" ,
        ...
        a."SoldBy" , a."SoldOn" , a."FarmerName" , a."LandArea"`
        const result = await db.sequelize.query(queryText);
        res.send(result[0]);
    } catch (e) {
        console.log(e);
...
exports.getAvailableTarget = async (req, res) => {
    try {
        ...
        as "TotalAvlTarget", a."DistributedPhyGen" + a."DistributedPhySCP" + a."DistributedPhyTASP" as "TotalDistributedTarget" 
        FROM "BlockTarget" a
...
exports.getclusterDemonstration = async (req, res) => {
    try {
        ...
        WHERE a."Block_Code" = '${req.payload.Block_Code}' AND a."Fin_Year" = '${req.query.Fin_Year}' AND a."schemeId" = '${req.query.schemeId}' 
        AND a."SubschemeId" = '${req.query.SubschemeId}' AND a."CompId"= '${req.query.CompId}'
...
exports.getDemonstrationStatusReport = async (req, res, next) => {
    try {
        const queryText = `SELECT a."DemostrationId",c."SubschemeName",d."CompName",a."SeedDistributionStatus",b."statusPhase1",b."statusPhase2",b."statusPhase3"
        FROM "DemonstrationPatchMaster" a
        LEFT JOIN "FarmerLandDetails" b ON a."DemostrationId" = b."DemostrationId"
        INNER JOIN "SubSchemeMaster" c ON a."SubschemeId" = c."SubschemeId"
        INNER JOIN "ComponentMaster" d ON a."CompId" = d."CompId" AND d."Fin_Year" = a."Fin_Year"
        WHERE "Block_Code"='${req.payload.Block_Code}' AND a."Fin_Year" = '${req.query.Fin_Year}' AND a."schemeId" = '${req.query.schemeId}'`;
        // console.log(queryText);
        const result = await db.sequelize.query(queryText);
        res.send(result[0]);
    } catch (e) {
...
exports.fieldDemonstrationIdPhaseDetails = async (req, res) => {
    try {
        const queryText1 = `SELECT b."DemostrationId",c."schemeName",d."CompName", b."statusPhase1",b."statusPhase2",b."statusPhase3"
        FROM "DemonstrationPatchMaster" a
        INNER JOIN "FarmerLandDetails" b ON a."DemostrationId" = b."DemostrationId"
        INNER JOIN "SchemeMaster" c ON a."schemeId" = c."schemeId"
        INNER JOIN "ComponentMaster" d ON a."CompId" = d."CompId" ND d."Fin_Year" = a."Fin_Year"
        WHERE "Block_Code"='${req.payload.Block_Code}' AND a."Fin_Year" = '${req.query.Fin_Year}' AND a."schemeId" = '${req.query.schemeId}'`
        const result = await db.sequelize.query(queryText1);
        res.send(result[0]);
    } catch (e) {
...
exports.getAllApprovedDealerSale = async (req, res, next) => {
    try {
        const queryText = `SELECT a."Block_Code", a."InvoiceNo", a."DemonstrationId" , a."Permit_NO" , a."FarmerId" ,a."FarmerName",a."LandArea", a."Farmer_Category",
                a."dealerLiscenseNo", a."SoldBy" , a."SoldOn" , a."FarmerName", a."verifyStatus",
                SUM(cast(a."totalPrice" as numeric)) AS TotalSalePrice ,
                ...
                WHERE a."verifyStatus" in ('Verifying','Approved_By_CDAO')  AND a."Block_Code" = '${req.payload.Block_Code}' AND a."DemonstrationId" = '${req.query.DemonstrationId}'
                AND c."baoAction" = 'Confirmed'
                GROUP BY a."InvoiceNo", a."DemonstrationId" , a."Permit_NO" , a."FarmerId" , a."Farmer_Category",a."dealerLiscenseNo",
                a."SoldBy" , a."SoldOn" , a."FarmerName" , a."schemeId" , a."Block_Code",a."LandArea",a."verifyStatus"`
                // console.log(queryText);
        const result = await db.sequelize.query(queryText);
        res.send(result[0]);
    } catch (e) {
...
exports.getAllDemonstrationData = async (req, res) => {
    try {
        const queryText = `SELECT a.* , ddd."schemeName" , dd."SubschemeName" , b."SubCropName" , c."Variety_Name" , bb."SubCropName" AS bpSubCropName ,
        cc."Variety_Name" AS bpCropVariety, d."CompName"
        FROM "DemonstrationPatchMaster" a
...
exports.getReturnedByCDAODealerSale = async (req, res, next) => {
    try {
        var DemonstrationId = req.query.DemonstrationId == '' ? 0 : req.query.DemonstrationId;
        ...
        const queryText = `SELECT a."Block_Code", a."InvoiceNo", a."DemonstrationId" , a."Permit_NO" , a."FarmerId" ,a."FarmerName",a."LandArea", a."Farmer_Category",
                ...
                INNER JOIN "DemonstrationPatchMaster" b ON a."DemonstrationId"= b."DemostrationId"
                INNER JOIN "DealerSaleToAction" c ON c."InvoiceNo" = a."InvoiceNo"
                WHERE a."verifyStatus" = 'Verifying'  AND a."Block_Code" = '${req.payload.Block_Code}'
                AND b."SubschemeId"='${req.query.SubschemeId}' AND  b."CompId"='${req.query.CompId}' AND ('0'='${DemonstrationId}' OR  a."DemonstrationId" = '${DemonstrationId}')
                AND c."ddaRemark" = 'Returned_To_BAO'
                AND a."Fin_year" = '${req.query.Fin_Year}'
                GROUP BY a."InvoiceNo", a."DemonstrationId" , a."Permit_NO" , a."FarmerId" , a."Farmer_Category",a."dealerLiscenseNo",
                a."SoldBy" , a."SoldOn" , a."FarmerName" , a."schemeId" , a."Block_Code",a."LandArea"`
        const result = await db.sequelize.query(queryText);
        res.send(result[0]);
    } catch (e) {
        console.log(e);
...
exports.getReturnedToDealerSale = async (req, res, next) => {
    try {
        var DemonstrationId = req.query.DemonstrationId == '' ? 0 : req.query.DemonstrationId;
        ...
        const queryText = `SELECT a."Block_Code", a."InvoiceNo", a."DemonstrationId" , a."Permit_NO" , a."FarmerId" ,a."FarmerName",a."LandArea", a."Farmer_Category",
                ...
                WHERE a."verifyStatus" = 'Verifying'  AND a."Block_Code" = '${req.payload.Block_Code}'
                AND b."SubschemeId"='${req.query.SubschemeId}' AND  b."CompId"='${req.query.CompId}' AND ('0'='${DemonstrationId}' OR  a."DemonstrationId" = '${DemonstrationId}')
                AND c."baoRemark" = 'Returned_To_Dealer'
                GROUP BY a."InvoiceNo", a."DemonstrationId" , a."Permit_NO" , a."FarmerId" , a."Farmer_Category",a."dealerLiscenseNo",
                a."SoldBy" , a."SoldOn" , a."FarmerName" , a."schemeId" , a."Block_Code",a."LandArea"`
        // console.log(queryText);
        const result = await db.sequelize.query(queryText);
...
exports.returnToDealer = async (req, res, next) => {
    const t = await sequelize.transaction();
    try {
        const data = req.body
        const promiseArray = []
        ...
        const result3 = await Promise.all(promiseArray)
        res.send({ message: "Successfully Returned to Dealer." })
        logController.addAuditLog(req.payload.user_id, req.protocol + '://' + req.get('host') + req.originalUrl, "Success", req.originalUrl.split("?").shift(), 'Update', req.method, req.socket.remoteAddress, parser.setUA(req.headers['user-agent']).getOS().name, parser.setUA(req.headers['user-agent']).getOS().version, parser.setUA(req.headers['user-agent']).getBrowser().name, parser.setUA(req.headers['user-agent']).getBrowser().version, req.device.type.toUpperCase())
...
exports.ReconfirmDealerSale = async (req, res, next) => {
    const t = await sequelize.transaction();
    try {
        const data = req.body
        const promiseArray = []
        ...
        data.forEach(e => {
                ...
                e.baoAction= 'Confirmed',
                e.bao_UserId= req.payload.user_id,
                e.baoTime= Date(),
                e.baoIp= req.socket.remoteAddress
                e.ddaRemark = null
                e.dda_UserId = null

                const udpateDealerSaleActionData = db.dealerSaleAction.update(e, { where: { InvoiceNo: e.InvoiceNo }, transaction: t })
                promiseArray1.push(udpateDealerSaleActionData)

                const udpateData = db.dealerSale.update({ verifyStatus: 'Verifying' }, { where: { InvoiceNo: e.InvoiceNo }, transaction: t })
                promiseArray.push(udpateData)
        })
        const result = await Promise.all(promiseArray1)
        const result3 = await Promise.all(promiseArray)
        res.send({ message: "Successfully Forwarded to CDAO." })
        logController.addAuditLog(req.payload.user_id, req.protocol + '://' + req.get('host') + req.originalUrl, "Success", req.originalUrl.split("?").shift(), 'Update', req.method, req.socket.remoteAddress, parser.setUA(req.headers['user-agent']).getOS().name, parser.setUA(req.headers['user-agent']).getOS().version, parser.setUA(req.headers['user-agent']).getBrowser().name, parser.setUA(req.headers['user-agent']).getBrowser().version, req.device.type.toUpperCase())
...
exports.returnedSaleCDAOToDealer = async (req, res, next) => {
    const t = await sequelize.transaction();
    try {
        const data = req.body     
        const promiseArray = []
        ...
        data.forEach(e => {
                ...
                e.baoTime= Date(),
                e.baoIp= req.socket.remoteAddress
                e.baoRemark = 'Returned_To_Dealer'
                e.ddaRemark = null
        ...
        const result3 = await Promise.all(promiseArray)
        res.send({ message: "Successfully Returned to Dealer." })
        logController.addAuditLog(req.payload.user_id, req.protocol + '://' + req.get('host') + req.originalUrl, "Success", req.originalUrl.split("?").shift(), 'Return', req.method, req.socket.remoteAddress, parser.setUA(req.headers['user-agent']).getOS().name, parser.setUA(req.headers['user-agent']).getOS().version, parser.setUA(req.headers['user-agent']).getBrowser().name, parser.setUA(req.headers['user-agent']).getBrowser().version, req.device.type.toUpperCase())
